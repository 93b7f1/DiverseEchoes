...
public class UploadImage implements IUpload {
    @Override
    public void upload(byte[] imageBytes, String folderPath, String fileName) throws InvalidFileExtensionException {
        String fileExtension = getFileExtension(fileName);
        if (!isValidExtension(fileExtension)) {
            throw new InvalidFileExtensionException("Extensão de arquivo inválida. Apenas jpg e png são permitidos.");
        }
        try {
            ...
            System.out.println("Imagem salva: " + localImagePath);

        } catch (IOException e) {
            System.out.println("Erro ao armazenar imagem: " + e.getMessage());
        }
    }
